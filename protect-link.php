...
function PostInfo($groupid, $postID, $Token, $userID, $token_admin)
{
    ...
    //Kiểm tra share
    $ShareApi = 'https://graph.facebook.com/v2.10/' . $groupid . '_' . $postID . '/sharedposts?fields=from{id}&limit=10000&access_token=' . $token_admin;
    $FindShare = get_info_by_curl($ShareApi);
    if (strpos($FindShare, $userID) !== false) {
        $Shared = true;
    }
...
if ($wap['Hash'] == $sHash) {
    ...
    */
    ...
    PostInfo($groupid, $FoundPostID, $accessToken, $userID, $token_admin);
    $Mem=true;
